...
//   console.log(songs)
  ...
  try {
    ...
    songList.forEach((song) => {
      ...
      if (!party.mustPlays.includes(song.spotify_id)) {
        party.mustPlays.push(song.spotify_id);
    ...
    songList.forEach((song) => {
        const index = party.doNotPlays.indexOf(song.spotify_id);
        if (index > -1) {
            party.doNotPlays.splice(index, 1);
        }
      });
